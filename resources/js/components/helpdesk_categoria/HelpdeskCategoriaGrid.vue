...
const source = '/admin/helpdesk_categoria/list';
...
const columns = ref([
    {name: 'nome', title: 'Nome', width: '10%', sort: 'nome', nowrap: true},
    {
        ...
        formatter: (value, row) => {
            let output = "";
            output += `<a href="javascript:;" data-json='{"id": "${value}"}' data-tooltip="Editar" data-action="popup" data-size="xl" data-component="helpdesk-categoria-form" data-title="Editar Tipo de Arquivo" class=" mx-1 action text-align-center tooltip tooltip--top"><i class="fa fa-pencil"></i></a>`;
...
const confirmRemove = async (data) => {
    ...
    try {
        await axios.delete(`/admin/helpdesk_categoria/${data.id}`);
